...
function formatCommandLine(line: string, currentTheme: string | undefined) {
  ...
  if (line.startsWith('$')) {
    return (
      <>
        <span className={currentTheme === 'dark' ? 'text-fuchsia-500' : 'text-fuchsia-600'}>dev@Rahul:~</span>
...
export function CliInterface({ onGuiCommand, onMinimize, onMaximize, onClose }: CliInterfaceProps) {
  ...
  useEffect(() => {
    ...
    setIsMounted(true);
    setOutput([
      isMobile ? 'Welcome to rahul.dev CLI! 👋' : ASCII_ART + '\nWelcome to my portfolio CLI! 👋',
